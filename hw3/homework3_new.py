...
def main(multiprocess=False, debug=False):
...
def path_search(start_position, end_position, primary_dict, presets, game_board, search_type, direction=0, debug=False):
    
    FORWARD_DIRECTION = 0
    BACKWARD_DIRECTION = 1
    globals()['forward_cost'] = 0
    globals()['backward_cost'] = 0
    globals()['forward_process_queue'] = []
    globals()['backward_process_queue'] = []
    ...
    while globals()['continue_comparator']:
        ...
        if direction == FORWARD_DIRECTION and globals()['forward_process_queue'] != []:
            process_queue += globals()['forward_process_queue']
            process_queue = sorted([list(x) for x in set(tuple(x) for x in process_queue)])
            #rint('--------Forward Process Queue--------')
            #print(process_queue[len(process_queue)-20:])
            #nput()
        elif direction == BACKWARD_DIRECTION and globals()['backward_process_queue'] != []:
            #input()
            process_queue += globals()['backward_process_queue']
            process_queue = sorted([list(x) for x in set(tuple(x) for x in process_queue)])
            #print('--------Backward Process Queue-------')
            #pprint(process_queue[len(process_queue)-20:])
            #input()
        ...
        if direction == FORWARD_DIRECTION:
            globals()['forward_cost'] = priority
        elif direction == BACKWARD_DIRECTION:
            globals()['backward_cost'] = priority
...
def node_comparator(start_position, end_position, forward_dict, backward_dict, game_board, search_type, debug=False):
    ...
    MIN_NODE_CONNECTIONS = 8
    #TIME_BUFFER = 0.01
    #MAX_LOOP_COUNT = 10
    LIST_OFFSET = 2
    #min_distance = LIST_OFFSET
    min_distance = int((((start_position['X']-end_position['X'])**2 + (start_position['Y']-end_position['Y'])**2)**.5)/2) + LIST_OFFSET
    ...
    forward_node_connector = []
    backward_node_connector = []
    ...
    while globals()['continue_comparator']:
        ...
        if len(forward_nodes) != 0 and len(backward_nodes) != 0:
            #print("deepest forward node:")
            #print(max(forward_nodes, key=len))
            #print("deepest backward node:")
            #print(max(backward_nodes, key=len))
            deepest_forward_node = max(forward_nodes, key=len)
            if deepest_forward_node[-1] == 'NODE':
                deepest_forward_id = deepest_forward_node[-2]
            else:
                deepest_forward_id = deepest_forward_node[-1]
            deepest_forward_id = deepest_forward_id.split(',')
            deepest_forward_mud = max(0, game_board[int(deepest_forward_id[1])][int(deepest_forward_id[0])])
            deepest_forward_node_cost = deepest_forward_node[0] - deepest_forward_mud
            deepest_forward_node_cost = deepest_forward_node[0]
            
            
            
            
            deepest_backward_node = max(backward_nodes, key=len)
            if deepest_backward_node[-1] == 'NODE':
                deepest_backward_id = deepest_backward_node[-2]
            else:
                deepest_backward_id = deepest_backward_node[-1]
            deepest_backward_id = deepest_backward_id.split(',')
            deepest_backward_mud = max(0, game_board[int(deepest_backward_id[1])][int(deepest_backward_id[0])])
            deepest_backward_node_cost = deepest_backward_node[0] - deepest_backward_mud
        '''
        ...
        for i in range(len(forward_nodes)):
            ...
            for j in range(len(backward_nodes)):
                ...
                if forward_node[-1] == 'NODE' and backward_node[0] == 'NODE':
                    if forward_node[-2] == backward_node[2]:
                        [forward_x, forward_y] = forward_node[-2].split(',')
                        
                        #print(forward_node[1:-1)
                        #print(backward_node[::-1][1:-1])
                        
                        #d_tools.del_from_dict(forward_dict, forward_node[1:-1])
                        #d_tools.del_from_dict(backward_dict, backward_node[::-1][1:-1])
                        
                        # Path includes cost 
                        forward_path = forward_node[:-1]+backward_node[2:]
                        ...
                        d_tools.get_from_dict(forward_dict, forward_node[1:-1])
                        d_tools.gen_nested_dict(backward_node[2:])
                        
                        d_tools.set_in_dict(forward_dict, forward_node[1:-1], " ".join(backward_node[2:]))
                        d_tools.set_in_dict(backward_dict, backward_node[1:-1], " ".join(forward_node[2:]))
                        
                        backward_node_replace = d_tools.get_from_dict(backward_dict, backward_node[::-1][1:])
                        '''
                        ...
                        if search_type == 'A*':
                            ...
                            overlap_cost = 0
                        forward_path[0] += forward_path[-1] - overlap_cost
                        forward_path.pop(-1)
                        forward_node_connector.append(forward_path)
                        
                        backward_path = [forward_path[0]] + forward_path[1:][::-1]
                        backward_node_connector.append(backward_path)
                        
                        [end_x, end_y] = backward_path[1].split(',')
                        forward_completed = d_tools.gen_nested_dict(backward_node[2:],{'C':forward_path[0], 'X':int(end_x), 'Y':int(end_y), 'M':game_board[int(end_y)][int(end_x)]})
                        
                        d_tools.set_in_dict(forward_dict, forward_node[1:-1], forward_completed)
                        #pprint(forward_completed)
                        
                '''
                elif forward_node[-1] != 'NODE' and backward_node[0] != 'NODE':
                    if forward_node[-1] == backward_node[0]:
                        #print("NOT NODE")
                        #print(forward_node[1:])
                        #print(backward_node[::-1][1:])
                        #print(forward_node)
                        #print(backward_node)
                        print("delete from:", forward_node[1:][:-1], forward_node[-1])
                        print("delete from:", backward_node[::-1][1:][:-1], backward_node[::-1][1:][-1])
                        
                        forward_node_replace = d_tools.get_from_dict(forward_dict, forward_node[1:])
                        backward_node_replace = d_tools.get_from_dict(backward_dict, backward_node[::-1][1:])
                        
                        forward_node_replace = {}
                        for key in backward_dict:
                            forward_node_replace
                        ...
                        #pprint(forward_dict)
                        #print(forward_node[1:-2])
                        #print(forward_node[-1])
                        #input()
                        d_tools.get_from_dict(forward_dict, forward_node[1:][:-1]).pop(forward_node[-1])
                        try:
                            d_tools.get_from_dict(backward_dict, backward_node[::-1][1:][:-1]).pop(backward_node[::-1][1:][-1])
        ...
        forward_node_connector = sorted([list(x) for x in set(tuple(x) for x in forward_node_connector)])
        backward_node_connector = sorted([list(x) for x in set(tuple(x) for x in backward_node_connector)])
        
        globals()['forward_process_queue'] = forward_node_connector
        globals()['backward_process_queue'] = backward_node_connector
        ...
        #    if globals()['path_found'] == '':
        #        time.sleep(2)
        #        print(len(node_connector))
        #        cost = node_connector[0][0]
        #        #print(cost, deepest_forward_node_cost, deepest_backward_node_cost)
        #        if cost < (deepest_forward_node_cost + deepest_backward_node_cost):   # NEEDS TO BE FIXED!!!
        ...
        if forward_nodes_length == len(forward_nodes) and backward_nodes_length == len(backward_nodes) and \
        ...
        if debug == True:
            print('--node_connector--')
            pprint(forward_node_connector)
            pprint(backward_node_connector)
            print('------------------')
            print('--forward_nodes--')
            pprint(forward_nodes)
            print('------------------')
...
class Dict_Tools:
    ...
    def gen_nested_dict(self, map_list, value):
        new_dict = {}
        for key in reversed(map_list):
            new_dict = {key: new_dict}
        
        self.set_in_dict(new_dict, map_list, value)
