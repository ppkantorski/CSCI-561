            ...
            for j in range(len(backward_nodes)):
                ...
                if forward_node[-1] == 'NODE' and backward_node[0] == 'NODE':
                    ...
                    if forward_node[-2] == backward_node[1]:
                        #print(forward_node[-2], backward_node[1])
                        ...
                        forward_cost = forward_node[0]
                        backward_cost = backward_node[-1]
                        forward_path = forward_node[1:-1]
                        backward_path = backward_node[1:-1]
                        path = forward_path+backward_path[1:]
                        
                        [middle_x, middle_y] = forward_path[-1].split(',')
                        
                        [end_x, end_y] = backward_path[0].split(',')
                        
                        #print(forward_node[1:-1])
                        #print(backward_node[1:-1][::-1])
                        #input()
                        
                        #d_tools.get_from_dict(globals()['forward_dict'], forward_node[1:-2]).pop(forward_node[-1])
                        #d_tools.get_from_dict(globals()['backward_dict'], backward_node[2:-1][::-1]).pop(backward_node[2:-1][::-1][-1])
                        
                        #d_tools.del_from_dict(globals()['forward_dict'], forward_node[1:-1])
                        #d_tools.del_from_dict(globals()['backward_dict'], backward_node[1:-1][::-1])
                        ...
                        if search_type == 'A*':
                            overlap_cost = max(0, game_board[int(middle_y)][int(middle_x)])
                        
                        path = [forward_cost+backward_cost-overlap_cost] + path
                        #path[0] += path[-1] - overlap_cost
                        #path.pop(-1)
                        if path not in node_connector:
                            node_connector.append(path)
                            
                            forward_completed = d_tools.gen_nested_dict(backward_path[1:],{'C':path[0], 'X':int(end_x), 'Y':int(end_y), 'M':game_board[int(end_y)][int(end_x)]})
                            backward_completed = d_tools.gen_nested_dict(forward_path[::-1][1:],{'C':path[0], 'X':int(start_x), 'Y':int(start_y), 'M':game_board[int(start_y)][int(start_x)]})
                            
                            d_tools.set_in_dict(forward_dict, forward_path, forward_completed)
                            d_tools.set_in_dict(backward_dict, backward_path[::-1], backward_completed)
                    ...
                    if forward_node[-1] == backward_node[0]:
                        ...
                        backward_path = backward_node[:-1]
                        path = forward_path+backward_path[1:]
                        
                        [middle_x, middle_y] = forward_path[-1].split(',')
                        
                        [end_x, end_y] = backward_path[-1].split(',')
                        [start_x, start_y] = forward_path[0].split(',')
