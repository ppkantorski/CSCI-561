...
def main(multiprocess=True, debug=False):
    ...
    if multiprocess == True and num_end_positions > 1:
        print("Multiprocessing is enabled.")
        with ProcessPoolExecutor(max_workers=min(num_end_positions, cpu_count())) as pool:
            output_data = list(pool.map(path_search, repeat(start_position), end_positions, repeat(presets), repeat(game_board), repeat(search_type)))
            #output_data = list(pool.map(bi_path_search, repeat(input_data), end_positions, repeat(search_type)))
    else:
        print("Multiprocessing is disabled.")
        output_data = []
        ...
        for end_position in end_positions:
            if counter == 0:
                ...
                output_data.append(path_search(start_position, end_position, presets, game_board, search_type, debug=False))
            print(output_data[len(output_data)-1])
            counter += 1
...
def gen_dict_from_queue(process_queue_entry, game_board, min_depth, end_position, search_type):
    
    #print('process_queue_entry', process_queue_entry)
    cost = process_queue_entry[0]
    positions = process_queue_entry[1:]
    positions.reverse()
    ...
    x_pos, y_pos = start_position.split(',')
    x_pos, y_pos = int(x_pos), int(y_pos)
    
    x_prev, y_prev = prev_position.split(',')
    x_prev, y_prev = int(x_prev), int(y_prev)
    
    curr_dist = int(((x_pos - end_position['X'])**2 + (y_pos - end_position['Y'])**2)**.5)
    prev_dist = int(((x_prev - end_position['X'])**2 + (y_prev - end_position['Y'])**2)**.5)
    
    if min_depth >= depth:
        if depth == 0:
            cost += curr_dist*scale
        elif min_depth == depth:
            cost -= prev_dist*scale
        else:
            cost -= prev_dist*scale
            cost += curr_dist*scale
        #cost += int((curr_dist- prev_dist)*scale)
    #if prev_dist > curr_dist and curr_dist != prev_dist:
    #cost += int((prev_dist-curr_dist)*scale)
    #    cost -= int(prev_dist*scale/4)
    
    #cost += int((curr_dist- prev_dist)*scale)
    
    #else:
    #    print(f'depth_surpassed... {min_depth} {depth}', positions)
    
    #print(start_position)
    x_pos, y_pos = start_position.split(',')
    x_pos, y_pos = int(x_pos), int(y_pos)
    
    new_dict[start_position] = {'C':cost, 'M':int(game_board[y_pos][x_pos]), 'X':x_pos, 'Y':y_pos}
    for position in positions[1:]:
        #print(position)
        new_dict[position] = {start_position:new_dict[start_position]}
        new_dict.pop(start_position)
...
def gen_dict_from_queue(process_queue_entry, game_board, min_depth, end_position, search_type):
    ...
    #min_depth = int(((x_pos - end_position['X'])**2 + (y_pos - end_position['Y'])**2)**.5) -1
    prev_depth = int(((x_prev - end_position['X'])**2 + (y_prev - end_position['Y'])**2)**.5) -1
    
    if min_depth >= depth:
        if depth == 0:
            cost += min_depth*scale
        else:
            cost -= min_depth*scale- (depth-1)*scale
            #cost -= prev_depth*scale
            cost += min_depth*scale
...
def path_search(start_position, end_position, presets, game_board, search_type, direction=0, debug=False):
    ...
    min_depth = int(((start_position['X'] - end_position['X'])**2 + (start_position['Y'] - end_position['Y'])**2)**.5)-1
    #print(min_depth)
    #min_depth = map_size['W'] + map_size['H']
    ...
    initial_keys = d_tools.get_all_keys(primary_dict, black_list=['X', 'Y', 'M', 'NODE'], map='C')
    for key in initial_keys:
        process_queue.put(key)
    #process_queue = deque(process_queue)
    
    #print(gen_dict_from_queue(process_queue[0], game_board))
    #
    #input()
    ...
    while True:
        ...
        primary_dict = gen_dict_from_queue(process, game_board, min_depth, end_position, search_type)
        ...
        if dict_id[len(dict_id)-1] == end_position_id:
            final_cost = priority
        ...
        #process_queue.get()
        #print(d_tools.get_all_keys(primary_dict, black_list=['X', 'Y', 'M', 'NODE'], map='C'))
        new_processes = d_tools.get_all_keys(primary_dict, black_list=['X', 'Y', 'M', 'NODE'], map='C')
        for new_process in new_processes:
            process_queue.put(new_process)
        #list(map(process_queue.put, new_processes))
        #process_queue = deque(sorted(process_queue))
...
def node_comparator(start_position, end_position, forward_dict, backward_dict, game_board, search_type, debug=False):
    ...
    while globals()['continue_comparator']:
        ...
        node_length = max(len(forward_nodes), len(backward_nodes))*3
        num_connected_nodes = len(node_connector)
        #print('num_connected_nodes:', num_connected_nodes, 'node_length:', node_length)
        
        #node_connection_length == len(node_connector) and 
        if (num_connected_nodes >= MIN_NODE_CONNECTIONS and globals()['continue_comparator'] == True and \
            num_connected_nodes > node_length) or (num_connected_nodes >= 1 and time.time()-time_in > 20): #and \
